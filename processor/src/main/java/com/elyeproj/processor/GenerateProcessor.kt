...
class GenerateProcessor : AbstractProcessor() {
    ...
    override fun getSupportedAnnotationTypes(): MutableSet<String> {
        return mutableSetOf(GenerateSource::class.java.canonicalName, CheckCamelSource::class.java.canonicalName, ChangeMyName::class.java.canonicalName)
    }
    ...
    override fun process(annotations: MutableSet<out TypeElement>, roundEnv: RoundEnvironment): Boolean {
        ...
        roundEnv.getElementsAnnotatedWith(ChangeMyName::class.java).forEach { fieldsElement ->

            if (fieldsElement.kind != ElementKind.FIELD) {
                printError("Can only be applied to field, element: $fieldsElement")
                return false
            } else {
                prepareChangeMyNameField(fieldsElement)
            }
    ...
    private fun prepareChangeMyNameField(fieldsElement: Element?) {

        val packageOfMethod = processingEnv.elementUtils.getPackageOf(fieldsElement).toString()

        val annotatedValue = fieldsElement?.getAnnotation(ChangeMyName::class.java)?.name

        val funcBuilder = FunSpec.builder("changeName")
                .addModifiers(KModifier.PUBLIC)
                .addParameter("newName", String::class)
                .addStatement("newName = %S", annotatedValue!!)
        ...
        FileSpec.builder(packageOfMethod,"NameGeneratedFunction").addFunction(funcBuilder.build())
                .build()
                .writeTo(file)
